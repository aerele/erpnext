...
class TestSubcontractingReceipt(ERPNextTestSuite):
	...
	@ERPNextTestSuite.change_settings("Buying Settings", {"auto_create_purchase_receipt": 1})
	def test_auto_create_purchase_receipt_with_no_reference_of_received_po_item(self):
		from erpnext.buying.doctype.purchase_order.test_purchase_order import create_purchase_order
		...
		# service item already received in full against the Purchase Order
		po.reload()
		po.items[0].db_set("received_qty", 12)
		...
		pr_qty = frappe.db.get_value("Purchase Receipt Item", {"parent": pr_name}, "qty")
		self.assertEqual(pr_qty, 6)
