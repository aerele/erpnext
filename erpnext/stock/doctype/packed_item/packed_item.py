...
def update_packed_item_stock_data(
	...
	stale_warehouses = dict((stale_item_warehouses or {}).get(main_item_row.get("name")) or {})
	if main_item_row.get("so_detail") and not stale_warehouses.get("warehouse"):
		# packed against a sales order and carried over: the row followed the bundle item
		# there, so let it go on following the one it is being delivered from
		stale_warehouses["warehouse"] = frappe.db.get_value(
			"Sales Order Item", main_item_row.so_detail, "warehouse"
		)
