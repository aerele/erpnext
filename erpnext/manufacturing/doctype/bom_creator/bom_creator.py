...
class BOMCreator(Document):
	...
	def set_conversion_factor(self):
		...
		for row in self.items:
			...
			if row.is_expandable and row.uom and row.uom != row.stock_uom:
				frappe.msgprint(
					_("Row {0}: UOM reset to {1}, a sub-assembly is measured in its stock UOM").format(
						row.idx, bold(row.stock_uom)
					),
					alert=True,
				)

			# the BOM generated for a sub-assembly takes its output qty from this row, and that BOM
			# is always in the item's stock UOM, so only raw materials can use another UOM
			if row.is_expandable or not row.uom:
