...
class TestBOMCreator(ERPNextTestSuite):
	...
	def test_sub_assembly_row_stays_in_stock_uom(self):
		doc = make_uom_bom_creator("Sheet BOM Sub Assembly Stock UOM")
		...
		doc.add_sub_assembly(
			...
			bom_item={
				...
				"qty": 2,
				"items": [
					{"item_code": "_Test Steel Sheet UOM", "qty": 2},
				],
			},
		)
		doc.reload()

		sub_assembly = next(row for row in doc.items if row.item_code == "_Test Steel Frame UOM")
		doc.edit_bom_creator(sub_assembly.name, {"uom": "_Test Box UOM"})
		doc.reload()

		# the BOM built for a sub-assembly takes its output qty from this row, so the row must
		# stay in the stock UOM
		sub_assembly = next(row for row in doc.items if row.item_code == "_Test Steel Frame UOM")
		self.assertEqual(sub_assembly.uom, "Nos")
		self.assertEqual(sub_assembly.conversion_factor, 1.0)
		self.assertEqual(sub_assembly.stock_qty, 2.0)

		doc.submit()
		doc.create_boms()

		bom = frappe.get_doc("BOM", {"bom_creator": doc.name, "item": "_Test Steel Frame UOM"})
		self.assertEqual(bom.uom, "Nos")
		self.assertEqual(bom.quantity, 2.0)
